...
function Shell() {
  ...
  const stats = useMemo(() => {
    ...
    const enabled = domains.filter(d => d.enabled === 1).length;
    const available = domains.filter(d => (d.last_status || "").toLowerCase() === "available").length;
    const rateLimited = domains.filter(d => (d.last_status || "") === "rate_limited").length;
    const errors = domains.filter(d => (d.last_status || "") === "error").length;
